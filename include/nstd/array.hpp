...
template<typename T, size_t N> T& array<T, N>::operator[](size_t index) {
    return _data[index];
}

template<typename T, size_t N> const T& array<T, N>::operator[](size_t index) const {
    return _data[index];
}

template<typename T, size_t N> T& array<T, N>::at(size_t index) {
    if (index >= N) {
...
template<typename T, size_t N> const T& array<T, N>::at(size_t index) const {
    if (index >= N) {
        throw std::out_of_range("array index out of range");
    }
    return _data[index];
...
template<typename T, size_t N> const T* array<T, N>::cbegin() const {
    return &_data[0];
}

template<typename T, size_t N> const T* array<T, N>::cend() const {
    return &_data[N];
}

template<typename T, size_t N> T* array<T, N>::rbegin() {
    return end() - 1;
}

template<typename T, size_t N> T* array<T, N>::rend() {
    return begin() - 1;
}

template<typename T, size_t N> const T* array<T, N>::rbegin() const {
    return end() - 1;
}

template<typename T, size_t N> const T* array<T, N>::rend() const {
    return begin() - 1;
}

template<typename T, size_t N> const T* array<T, N>::crbegin() const {
    return end() - 1;
}

template<typename T, size_t N> const T* array<T, N>::crend() const {
    return begin() - 1;
}
...
template<typename T, size_t N> auto array<T, N>::operator<=>(const array& other) const {
...
template<typename T, size_t N> bool array<T, N>::operator==(const array& other) const {
    for (size_t i{}; i < N; ++i) {
        if (_data[i] != other._data[i]) {
            return false;
        }
