...
namespace nstd {
	...
	template<typename T>
	constexpr void default_delete<T>::operator()(T* ptr) const noexcept {
		delete ptr;
	}
	...
	template<typename T, typename Deleter>
	constexpr unique_ptr<T, Deleter>::unique_ptr(T* ptr, const Deleter& d) noexcept : _ptr{ ptr }, _deleter{ d } {}

	template<typename T, typename Deleter>
	constexpr unique_ptr<T, Deleter>::unique_ptr(T* ptr, Deleter&& d) noexcept : _ptr{ ptr }, _deleter{ std::move(d) } {}
	...
	template<typename T, typename Deleter>
	constexpr T* unique_ptr<T, Deleter>::get() const noexcept {
		return _ptr;
	}
	...
	template<typename T, typename Deleter>
	constexpr T* unique_ptr<T, Deleter>::release() noexcept {
		T* temp = _ptr;
		_ptr = nullptr;
		return temp;
	}
	...
	public:
		...
		template<typename U>
		constexpr explicit unique_ptr(U* ptr) noexcept
			requires(std::is_same_v<U, T>);

		template<typename U>
		constexpr unique_ptr(U* ptr, const Deleter& d) noexcept
			requires(std::is_same_v<U, T>);

		template<typename U>
		constexpr unique_ptr(U* ptr, Deleter&& d) noexcept
			requires(std::is_same_v<U, T>);;
	...
	template<typename T, typename Deleter>
	template<typename U>
	constexpr unique_ptr<T[], Deleter>::unique_ptr(U* ptr) noexcept
		requires(std::is_same_v<U, T>)
	: _ptr{ ptr } {
	}
