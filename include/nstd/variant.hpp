...
template<typename Target, typename... List> struct type_index<Target, Target, List...> {
...
template<size_t CurrentIndex, size_t TargetIndex, typename... List> struct index_type;
...
public:
    ...
    template<typename T> variant(T&& value) requires(!std::is_same_v<std::decay_t<T>, variant>) {
    ...
    template<typename Visitor> void visit(Visitor&& visitor) {
        ...
        static constexpr nstd::array<void (*)(void*, Visitor&&), sizeof...(Types)> table{
            (+[](void* buffer, Visitor&& vis) { vis(*reinterpret_cast<Types*>(buffer)); })...};
...
private:
    template<typename T> static void _destroyer(void* ptr) {
        reinterpret_cast<T*>(ptr)->~T();
    }

    static constexpr nstd::array<void (*)(void*), sizeof...(Types)> _destroyers{
        _destroyer<Types>...};
